...
public class StreetAdapter extends RecyclerView.Adapter<StreetAdapter.ViewHolder> {
    ...
    public StreetAdapter(List<String> beanList) {
    ...
    public void onBindViewHolder(final ViewHolder holder, final int position) {
       ...
       holder.product_check_box.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
           ...
           public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
               if(isChecked){
                   selectedProductList.add(beanList.get(position));
               }else{
                   if(selectedProductList.contains(beanList.get(position))){
                       selectedProductList.remove(beanList.get(position));
                   }
               }
           }
       });
